...
public class Clock2Tester{
    ...
    public static void main(String[] args) {
        ...
        // Shape dimensions
        ClockShape2 shape = new ClockShape2(100, 0, 600);

        ShapeIcon icon = new ShapeIcon(shape, WIDTH, HEIGHT);

        final JLabel label = new JLabel(icon);
        JTextArea[] areaArray = new JTextArea[12];
        for(int i = 0; i<12; i++){
            areaArray[i] = new JTextArea(" " + String.valueOf(i + 1));
            areaArray[i].setBounds((int)(390 + 275*Math.cos(Math.PI*((3-i)/6.))),(int)(295 - 275*Math.sin(Math.PI*((3-i)/6.))),20,20);
            areaArray[i].setBackground(Color.PINK);
            label.add(areaArray[i]);
        }
