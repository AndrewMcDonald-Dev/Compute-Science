...
public class ExprEvaluator {
  ...
  // Since expressions are evaluated one at a time, all evaluations can use the
  // same two stacks.
  ...
  public static String expressionOutput(double x) {
    return (x == (double) Math.round(x)) ? "" + (int) x : "" + x;
  }
   ...
   * evaluated by popping the current operator from A, its 2 operands from B, and
   * by then performing the operation and pushing the result onto B as a double
   * value.
   */
  ...
  /*
   * In this method, a parenthesized subexpression is evaluated by evaluating its
   * operations on Stack A top-down until a right parenthesis is encountered on
   * stack A; the right parenthesis is also popped from A
   */
  ...
  // This method compares the current operator token in the input string to the
  // operator
  ...
  private double formNum() {
    ...
    } while ((p < e.length()) && (((d <= '9') && (d >= '0')) || (d == '_') || (d == '.')));// check for a valid
                                                                                           // character
  ...
  // This method uses the 2-stack approach to evaluate an expression (from right
  // to left).
  // The result is rounded to 7 decimal places and displayed as explained in
  // expressionOutput() above.
  // The original double value (unrounded) is returned to the calling program.
  // The code could be made more efficient (but more difficult to read) by using
  // if-else-if-else...
  ...
  public double evaluator() {
    ...
    do {
      ...
      // if the token is the rightmost digit of a number or a decimal point on the
      // right,
