...
public class CircularTracksShape implements Moveable {
  ...
  public void translate(int dx, int dy) {
    ...
    for (int i = 0; i <= n - 1; i = i + 2) {
      x[i] = (int) ((290 + Math.cos(Math.toRadians(angles[i])) * radius[i]));
      y[i] = (int) ((290 + Math.sin(Math.toRadians(angles[i])) * radius[i]));
      ...
      // y[i] = y[i] + dy;
      // } else {
      // x[i] = x[i] - dx; // train is on bottom of track, move left
      // }
    } // for
