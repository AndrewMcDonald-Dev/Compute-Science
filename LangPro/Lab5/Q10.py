...
def main():
   ...
   def fileReader(fileLocation):
      global source
      try:
         infile = open(sys.argv[fileLocation], 'r')
         source = infile.read()  # read source program
      except IOError:
         print('Cannot read input file ' + sys.argv[fileLocation])
         sys.exit(1)

   if len(sys.argv) == 2:   # check if correct number of cmd line args
      fileReader(1)

   elif len(sys.argv) == 3:   # Checks for arguments
      if sys.argv[1] == '-traceon':
         trace = True
         fileReader(2)
      elif sys.argv[2] == '-traceon':
         trace = True
         fileReader(1)
      else:
         raise RuntimeError('Bad flag used.')
